...
class GroqService extends LlmService {
  ...
  Future<Map<String, dynamic>> analyzeCode({
    ...
    // Select models based on input type - all text models for now
    final List<String> modelList = ApiConstants.groqTextModels;
    ...
    for (int i = 0; i < modelList.length; i++) {
      try {
        ...
        if (i == modelList.length - 1) {
          throw Exception(
            'All Groq models failed. Last error: ${e.toString()}',
          );
        }
        continue;
      }
    }
    throw Exception('Failed to analyze code with any available Groq model');
  }
  ...
  Future<Map<String, dynamic>> _tryAnalyzeWithModel({
    ...
    try {
      ...
      if (choices == null || choices is! List || choices.isEmpty) {
        throw Exception('No choices field in Groq API response');
      }
      ...
      try {
        ...
        throw Exception(
          'Failed to decode JSON from Groq: $e\nContent: $jsonContent',
        );
      }
    } catch (e) {
      throw Exception('Groq Model $model failed: ${e.toString()}');
    }
  }
}
